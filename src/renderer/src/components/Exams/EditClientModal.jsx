...
const EditClientModal = ({ isOpen, onClose, client, calculateAge, onSave }) => {
  ...
  const [trafficLawTest, setTrafficLawTest] = useState(client.tests?.trafficLawTest || { passed: false, attempts: 0, lastAttemptDate: null })
  const [manoeuvresTest, setManoeuvresTest] = useState(client.tests?.manoeuvresTest || { passed: false, attempts: 0, lastAttemptDate: null })
  const [drivingTest, setDrivingTest] = useState(client.tests?.drivingTest || { passed: false, attempts: 0, lastAttemptDate: null })

  // Disable driving test input if the client is under 19
  const isDrivingTestDisabled = age < 19
